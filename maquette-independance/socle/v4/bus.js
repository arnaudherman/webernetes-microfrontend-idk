...
/**
 * Socle partagé — version 4.0.
 *
 * API STRICTEMENT IDENTIQUE à la 3.0. Une seule chose change : le numéro de version,
 * donc l'URL, donc l'instance.
 *
 * C'est délibéré, et c'est ce que la page de coexistence doit démontrer : deux URL
 * distinctes produisent deux INSTANCES distinctes, donc deux états distincts, même
 * quand le code est identique au caractère près. Il n'y a pas de faute
 * d'implémentation à chercher dans ce fichier.
 *
 * Y mettre une rupture d'API en plus brouillerait la démonstration : la page porterait
 * deux enseignements à la fois, et l'observateur attribuerait le silence au mauvais.
 *
