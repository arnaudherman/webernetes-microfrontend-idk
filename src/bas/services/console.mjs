...
import { origine, ORIGINES_PAGE, PORTS } from "./adresses.mjs";
import { appeler, EN_TETE_SONDE } from "./socle-service.mjs";
 ...
 *
 * @type {readonly { nom: import("./adresses.mjs").NomService, port: number, fichier: string }[]}
 */
const SERVICES = [
  { nom: "taches", port: PORTS.taches, fichier: "taches.mjs" },
  { nom: "charges", port: PORTS.charges, fichier: "charges.mjs" },
  { nom: "passerelle", port: PORTS.passerelle, fichier: "passerelle.mjs" },
];
 ...
 * @property {import("./adresses.mjs").NomService} nom
...
function demarrerService(nom) {
  ...
  const enfant = spawn(process.execPath, [join(RACINE, service.fichier)], {
